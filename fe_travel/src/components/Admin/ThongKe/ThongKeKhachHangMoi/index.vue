<template>
    <div style="padding: 20px; background: #f5f7fa; min-height: 100vh;">
        ...
        <div style="margin-bottom: 30px;">
            <h1 style="font-size: 2rem; font-weight: 700; color: #333; margin: 0;">👥 Thống Kê Khách Hàng Mới</h1>
            <p style="color: #666; margin: 5px 0 0 0;">Phân tích số lượng khách hàng mới đăng ký theo ngày</p>
        </div>
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 20px; margin-bottom: 30px;">
            ...
            <div style="background: white; border-radius: 12px; padding: 20px; box-shadow: 0 2px 8px rgba(0,0,0,0.08); border-left: 4px solid #667eea;">
                <div style="display: flex; align-items: center; justify-content: space-between;">
                    <div>
                        <p style="color: #999; font-size: 0.85rem; margin: 0 0 5px 0; text-transform: uppercase; font-weight: 600;">Tổng Khách Hàng Mới</p>
                        <h3 style="color: #333; font-size: 1.8rem; margin: 0; font-weight: 700;">{{ summary.totalNewCustomers }}</h3>
                    </div>
            ...
            <div style="background: white; border-radius: 12px; padding: 20px; box-shadow: 0 2px 8px rgba(0,0,0,0.08); border-left: 4px solid #764ba2;">
                <div style="display: flex; align-items: center; justify-content: space-between;">
                    <div>
                        <p style="color: #999; font-size: 0.85rem; margin: 0 0 5px 0; text-transform: uppercase; font-weight: 600;">Trung Bình/Ngày</p>
                        <h3 style="color: #333; font-size: 1.8rem; margin: 0; font-weight: 700;">{{ Math.round(summary.averagePerDay) }}</h3>
                    </div>
            ...
            <div style="background: white; border-radius: 12px; padding: 20px; box-shadow: 0 2px 8px rgba(0,0,0,0.08); border-left: 4px solid #06b6d4;">
                <div style="display: flex; align-items: center; justify-content: space-between;">
                    <div>
                        <p style="color: #999; font-size: 0.85rem; margin: 0 0 5px 0; text-transform: uppercase; font-weight: 600;">Ngày Cao Nhất</p>
                        <h3 style="color: #333; font-size: 1.2rem; margin: 0; font-weight: 700;">{{ summary.peakDay }}</h3>
                    </div>
        ...
        <div style="background: white; border-radius: 12px; padding: 20px; box-shadow: 0 2px 8px rgba(0,0,0,0.08); margin-bottom: 30px;">
            <h4 style="color: #333; margin: 0 0 15px 0; font-weight: 700;">Khách Hàng Mới Theo Ngày</h4>
            <canvas id="chartKhachHangMoi" style="max-height: 400px;"></canvas>
        </div>

        <!-- Data Table -->
        <div style="background: white; border-radius: 12px; padding: 20px; box-shadow: 0 2px 8px rgba(0,0,0,0.08);">
            <h4 style="color: #333; margin: 0 0 15px 0; font-weight: 700;">Chi Tiết Khách Hàng Mới</h4>
            <div style="overflow-x: auto;">
                <table style="width: 100%; border-collapse: collapse; font-size: 0.9rem;">
                    <thead>
                        <tr style="background: #f8f9fa; border-bottom: 2px solid #e2e8f0;">
                            <th style="padding: 12px; text-align: left; color: #666; font-weight: 600;">Ngày</th>
                            <th style="padding: 12px; text-align: right; color: #666; font-weight: 600;">Số Khách Hàng Mới</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr v-for="(item, index) in tableData" :key="index" style="border-bottom: 1px solid #e2e8f0; transition: background 0.2s;"
                            ...
                            <td style="padding: 12px; color: #333;">{{ formatDate(item.ngay_dang_ky) }}</td>
                            <td style="padding: 12px; text-align: right; color: #667eea; font-weight: 600;">{{ item.so_khach_hang_moi }}</td>
                        </tr>
                        <tr v-if="tableData.length === 0" style="border-bottom: 1px solid #e2e8f0;">
                            <td colspan="2" style="padding: 20px; text-align: center; color: #999;">Không có dữ liệu</td>
                        </tr>
                    </tbody>
                </table>
            </div>
    ...
    methods: {
        ...
        async loadStatistics() {
            ...
            try {
                ...
                const response = await axios.post(apiUrl('admin/thong-ke/khach-hang-moi'), {
                ...
                if (response.data.status) {
                    ...
                    // Calculate summary
                    this.summary.totalNewCustomers = this.tableData.reduce((sum, item) => sum + (Number(item.so_khach_hang_moi) || 0), 0);
                    this.summary.averagePerDay = this.tableData.length > 0 ? this.summary.totalNewCustomers / this.tableData.length : 0;
                    
                    // Find peak day
                    if (this.tableData.length > 0) {
                        const maxDay = this.tableData.reduce((max, item) => 
                            Number(item.so_khach_hang_moi) > Number(max.so_khach_hang_moi) ? item : max
                        );
                        this.summary.peakDay = this.formatDate(maxDay.ngay_dang_ky) + ` (${maxDay.so_khach_hang_moi})`;
                    }
        ...
        renderChart(data) {
            const ctx = document.getElementById('chartKhachHangMoi');
            if (!ctx) return;
