<template>
    <div style="padding: 20px; background: #f5f7fa; min-height: 100vh;">
        ...
        <div style="margin-bottom: 30px;">
            <h1 style="font-size: 2rem; font-weight: 700; color: #333; margin: 0;">✈️ Thống Kê Hiệu Suất Tour</h1>
            <p style="color: #666; margin: 5px 0 0 0;">Phân tích hiệu suất bán hàng - Chỉ tính vé từ hóa đơn có trang_thai = 1</p>
        </div>
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 20px; margin-bottom: 30px;">
            ...
            <div style="background: white; border-radius: 12px; padding: 20px; box-shadow: 0 2px 8px rgba(0,0,0,0.08); border-left: 4px solid #667eea;">
                <div style="display: flex; align-items: center; justify-content: space-between;">
                    <div>
                        <p style="color: #999; font-size: 0.85rem; margin: 0 0 5px 0; text-transform: uppercase; font-weight: 600;">Tổng Tour</p>
                        <h3 style="color: #333; font-size: 1.8rem; margin: 0; font-weight: 700;">{{ summary.totalTours }}</h3>
                    </div>
            ...
            <div style="background: white; border-radius: 12px; padding: 20px; box-shadow: 0 2px 8px rgba(0,0,0,0.08); border-left: 4px solid #10b981;">
                <div style="display: flex; align-items: center; justify-content: space-between;">
                    <div>
                        <p style="color: #999; font-size: 0.85rem; margin: 0 0 5px 0; text-transform: uppercase; font-weight: 600;">Doanh Thu Dự Kiến</p>
        ...
        <div style="background: white; border-radius: 12px; padding: 20px; box-shadow: 0 2px 8px rgba(0,0,0,0.08); margin-bottom: 30px;">
            <h4 style="color: #333; margin: 0 0 15px 0; font-weight: 700;">Vé Bán Ra Theo Tour</h4>
            <canvas id="chartTourPerformance" style="max-height: 400px;"></canvas>
        </div>

        <!-- Data Table -->
        <div style="background: white; border-radius: 12px; padding: 20px; box-shadow: 0 2px 8px rgba(0,0,0,0.08);">
            <h4 style="color: #333; margin: 0 0 15px 0; font-weight: 700;">Chi Tiết Tour</h4>
            <div style="overflow-x: auto;">
                <table style="width: 100%; border-collapse: collapse; font-size: 0.9rem;">
                    <thead>
                        <tr style="background: #f8f9fa; border-bottom: 2px solid #e2e8f0;">
                            <th style="padding: 12px; text-align: left; color: #666; font-weight: 600;">Tên Tour</th>
                            <th style="padding: 12px; text-align: right; color: #666; font-weight: 600;">Số Vé Bán Ra</th>
                            <th style="padding: 12px; text-align: right; color: #666; font-weight: 600;">Doanh Thu Dự Kiến</th>
                            <th style="padding: 12px; text-align: center; color: #666; font-weight: 600;">Xếp Hạng</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr v-for="(item, index) in tableData" :key="index" style="border-bottom: 1px solid #e2e8f0; transition: background 0.2s;"
                            ...
                            <td style="padding: 12px; color: #333;">{{ item.ten_tour }}</td>
                            <td style="padding: 12px; text-align: right; color: #666;">{{ item.so_luong_ve_ban_ra }}</td>
                            <td style="padding: 12px; text-align: right; color: #667eea; font-weight: 600;">{{ formatVND(item.doanh_thu_du_kien) }}</td>
                            <td style="padding: 12px; text-align: center;">
                                <span style="display: inline-block; background: #667eea; color: white; padding: 4px 12px; border-radius: 20px; font-weight: 600; font-size: 0.85rem;">
                                    #{{ index + 1 }}
                                </span>
                            </td>
                        </tr>
    ...
    methods: {
        ...
        async loadStatistics() {
            ...
            try {
                ...
                const response = await axios.post(apiUrl('admin/thong-ke/tour'), {
                ...
                if (response.data.status) {
                    ...
                    // Calculate summary
                    this.summary.totalTours = this.tableData.length;
                    this.summary.totalTickets = this.tableData.reduce((sum, item) => sum + (Number(item.so_luong_ve_ban_ra) || 0), 0);
                    this.summary.totalRevenue = this.tableData.reduce((sum, item) => sum + (Number(item.doanh_thu_du_kien) || 0), 0);
        ...
        renderChart(data) {
            const ctx = document.getElementById('chartTourPerformance');
            if (!ctx) return;
