<template>
    <div style="padding: 20px; background: #f5f7fa; min-height: 100vh;">
        ...
        <div style="margin-bottom: 30px;">
            <h1 style="font-size: 2rem; font-weight: 700; color: #333; margin: 0;">💳 Thống Kê Chi Tiêu Khách Hàng</h1>
            <p style="color: #666; margin: 5px 0 0 0;">Phân tích chi tiêu của từng khách hàng</p>
        </div>
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 20px; margin-bottom: 30px;">
            ...
            <div style="background: white; border-radius: 12px; padding: 20px; box-shadow: 0 2px 8px rgba(0,0,0,0.08); border-left: 4px solid #667eea;">
                <div style="display: flex; align-items: center; justify-content: space-between;">
                    <div>
                        <p style="color: #999; font-size: 0.85rem; margin: 0 0 5px 0; text-transform: uppercase; font-weight: 600;">Tổng Chi Tiêu</p>
                        <h3 style="color: #333; font-size: 1.8rem; margin: 0; font-weight: 700;">{{ formatVND(summary.totalSpending) }}</h3>
                    </div>
            ...
            <div style="background: white; border-radius: 12px; padding: 20px; box-shadow: 0 2px 8px rgba(0,0,0,0.08); border-left: 4px solid #764ba2;">
                <div style="display: flex; align-items: center; justify-content: space-between;">
                    <div>
                        <p style="color: #999; font-size: 0.85rem; margin: 0 0 5px 0; text-transform: uppercase; font-weight: 600;">Số Khách Hàng</p>
                        <h3 style="color: #333; font-size: 1.8rem; margin: 0; font-weight: 700;">{{ summary.numCustomers }}</h3>
                    </div>
            ...
            <div style="background: white; border-radius: 12px; padding: 20px; box-shadow: 0 2px 8px rgba(0,0,0,0.08); border-left: 4px solid #06b6d4;">
                <div style="display: flex; align-items: center; justify-content: space-between;">
                    <div>
                        <p style="color: #999; font-size: 0.85rem; margin: 0 0 5px 0; text-transform: uppercase; font-weight: 600;">Chi Tiêu Trung Bình</p>
                        <h3 style="color: #333; font-size: 1.8rem; margin: 0; font-weight: 700;">{{ formatVND(summary.averageSpending) }}</h3>
                    </div>
        ...
        <div style="background: white; border-radius: 12px; padding: 20px; box-shadow: 0 2px 8px rgba(0,0,0,0.08); margin-bottom: 30px;">
            <h4 style="color: #333; margin: 0 0 15px 0; font-weight: 700;">Chi Tiêu Khách Hàng (Top 10)</h4>
            <canvas id="chartChiTieuKhachHang" style="max-height: 400px;"></canvas>
        </div>

        <!-- Data Table -->
        <div style="background: white; border-radius: 12px; padding: 20px; box-shadow: 0 2px 8px rgba(0,0,0,0.08);">
            <h4 style="color: #333; margin: 0 0 15px 0; font-weight: 700;">Chi Tiết Chi Tiêu Khách Hàng</h4>
            <div style="overflow-x: auto;">
                <table style="width: 100%; border-collapse: collapse; font-size: 0.9rem;">
                    <thead>
                        <tr style="background: #f8f9fa; border-bottom: 2px solid #e2e8f0;">
                            <th style="padding: 12px; text-align: left; color: #666; font-weight: 600;">Tên Khách Hàng</th>
                            <th style="padding: 12px; text-align: right; color: #666; font-weight: 600;">Số Đơn Hàng</th>
                            <th style="padding: 12px; text-align: right; color: #666; font-weight: 600;">Tổng Chi Tiêu</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr v-for="(item, index) in tableData" :key="index" style="border-bottom: 1px solid #e2e8f0; transition: background 0.2s;"
                            ...
                            <td style="padding: 12px; color: #333;">{{ item.ho_va_ten }}</td>
                            <td style="padding: 12px; text-align: right; color: #666;">{{ item.so_don_hang }}</td>
                            <td style="padding: 12px; text-align: right; color: #667eea; font-weight: 600;">{{ formatVND(item.tong_chi_tieu) }}</td>
                        </tr>
    ...
    methods: {
        ...
        async loadStatistics() {
            ...
            try {
                ...
                const response = await axios.post(apiUrl('admin/thong-ke/chi-tieu-khach-hang'), {
                ...
                if (response.data.status) {
                    ...
                    // Calculate summary
                    this.summary.totalSpending = this.tableData.reduce((sum, item) => sum + (Number(item.tong_chi_tieu) || 0), 0);
                    this.summary.numCustomers = this.tableData.length;
                    this.summary.averageSpending = this.summary.numCustomers > 0 ? this.summary.totalSpending / this.summary.numCustomers : 0;
        ...
        renderChart(data) {
            const ctx = document.getElementById('chartChiTieuKhachHang');
            if (!ctx) return;
